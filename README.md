...
3. Inserire i propri dati nella form (email e password).

4. Nel campo lezione bisogna scrivere il titolo, o una parte di esso, del video che da cui si intende iniziare, tenendo
...
5. Una volta premuto il tasto corrispondente al tipo di corso (con o senza pulsante per il video successivo) verrà aperta
 una nuova sessione del browser, ed una volta che il programma arriverà alla pagina dove viene riprodotto il video scelto
 ed avrà messo in play quest'ultimo, sarà possibile clickare manualmente il tasto del player per mutare il video senza pericolo
 di intaccare il funzionamento del programma.
...
- Per sicurezza sono stati aggiunti alcuni secondi di delay tra la fine del video in riproduzione e l'avvio del successivo, quindi
 dare il tempo all'applicazione di passare autonomamente al prossimo video.


## Risoluzione dei Problemi
- Se riscontri problemi durante l'utilizzo, assicurati di seguire correttamente le istruzioni.

- Se l'applicazione si blocca o si verifica un errore, prova a riavviarla.
